...
wpi::array<frc::SwerveModuleState, 4> SwerveDrive::getRealModuleStates() {
    ...
    // return moduleStates;
    //avoiding compile bugs but PLEASE dont call right now
     wpi::array<frc::SwerveModuleState, 4> moduleStates = {
        flModule_.getState(), frModule_.getState(), flModule_.getState(), frModule_.getState() };
    return moduleStates;
}
...
void SwerveDrive::Periodic(units::meters_per_second_t dx, units::meters_per_second_t dy, units::radians_per_second_t dtheta, double turretAngle) {
    ...
    frc::SmartDashboard::PutNumber("fl can", flModule_.getYaw());
    frc::SmartDashboard::PutNumber("fr can", frModule_.getYaw());
    // frc::SmartDashboard::PutNumber("bl can", blModule_.getYaw());
    // frc::SmartDashboard::PutNumber("br can", brModule_.getYaw());
...
void SwerveDrive::drive(units::meters_per_second_t dx, units::meters_per_second_t dy, units::radians_per_second_t dtheta, double turretAngle) {
    ...
    std::cout << "speed error: " << flModule_.getVelocityMPS().value() - fl_opt.speed.value() << "\n";
    std::cout << "speed volts: " << std::clamp(
...
void SwerveDrive::differentialDrive(units::volt_t leftVolts, units::volt_t rightVolts) {
    frc::SmartDashboard::PutNumber("Left Volts", leftVolts.value());
    frc::SmartDashboard::PutNumber("Right Volts", rightVolts.value());

    //voltage constraints
    double lVolts = std::clamp(leftVolts, units::volt_t{-GeneralConstants::MAX_VOLTAGE}, units::volt_t{-GeneralConstants::MAX_VOLTAGE}).value();
    double rVolts = std::clamp(rightVolts, units::volt_t{-GeneralConstants::MAX_VOLTAGE}, units::volt_t{-GeneralConstants::MAX_VOLTAGE}).value();

    flModule_.setSpeedMotorVolts(lVolts);
  //  blModule_.setSpeedMotorVolts(lVolts);

    frModule_.setSpeedMotorVolts(rVolts);
 //   brModule_.setSpeedMotorVolts(rVolts);
...
/**
 * Used for tuning speed PID values
**/
