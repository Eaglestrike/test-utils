...
tuple<double, double, double> TrajectoryCalc::getProfile(double time)
{
    double tP, tV, tA;
...
double TrajectoryCalc::calcPower(double pos, double vel, double time)
{
    tuple<double, double, double> profile = getProfile(time);
    ...
    if(power > GeneralConstants::MAX_VOLTAGE)
    {
        return GeneralConstants::MAX_VOLTAGE;
    }
    else if(power < -GeneralConstants::MAX_VOLTAGE)
    {
        return -GeneralConstants::MAX_VOLTAGE;
    }
    return power;
...
double TrajectoryCalc::getTotalTime()
{
    return accelTime_ + deccelTime_ + cruiseTime_;
}